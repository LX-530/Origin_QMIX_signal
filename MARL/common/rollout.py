...
class RolloutWorker:
    ...
    def generate_episode(self, epoch,fire_info, episode_num=None, evaluate=False):
        ...
        while not terminated and step < self.episode_limit:
            ...
            static_action = [1, 2, 2, 0, 1, 1, 1, 1, 2, 0, 0, 0, 2, 3, 3, 1, 1, 2]
        ...
        # 评估时聚合本 episode 的统一指标，供 Runner 写入 qmix_eval_metrics.csv（与 CDQN 指标可比）
        if evaluate:
            static_cum = float(sum(x[0] for x in episode_rewards)) if episode_rewards else 0.0
            fire_cum = float(sum(x[1] for x in episode_rewards)) if episode_rewards else 0.0
            cong_cum = float(sum(x[2] for x in episode_rewards)) if episode_rewards else 0.0
            self.last_eval_metrics = {
                'completion': int(bool(terminated)),   # 是否在 episode_limit 内真正疏散完成
                'evac_steps': step,
