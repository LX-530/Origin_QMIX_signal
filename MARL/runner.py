...
class Runner:
    ...
    def evaluate(self,epoch):
        win_number = 0
        episode_rewards = 0
        agg = {'completion': 0.0, 'evac_steps': 0.0, 'reward_sum': 0.0,
               'static_cum': 0.0, 'fire_cum': 0.0, 'cong_cum': 0.0}
        for e_epoch in range(self.args.evaluate_epoch):
            ...
            if win_tag:
                win_number += 1
            m = getattr(self.rolloutWorker, 'last_eval_metrics', None)
            if m:
                for k in agg:
                    agg[k] += m[k]
        n = max(1, self.args.evaluate_epoch)
        self._log_eval_metrics(epoch, {k: agg[k] / n for k in agg})
        # 返回的是平均获胜次数和平均奖励
        # print(for_gant)
        return win_tag, episode_rewards / self.args.evaluate_epoch

    def _log_eval_metrics(self, epoch, m):
        """写统一格式的评估指标 CSV，与 CDQN 的 cascade_eval_metrics.csv 对齐，供 analyze_compare.py 对比。"""
        path = self.args.save_path + '/historydata/qmix_eval_metrics.csv'
        header = ['epoch', 'completion', 'evac_steps', 'reward_sum', 'fire_cum', 'cong_cum', 'static_cum']
        write_header = not os.path.exists(path)
        with open(path, 'a', newline='') as f:
            w = csv.writer(f)
            if write_header:
                w.writerow(header)
            w.writerow([epoch, round(m['completion'], 4), round(m['evac_steps'], 4),
                        round(m['reward_sum'], 4), round(m['fire_cum'], 4),
                        round(m['cong_cum'], 4), round(m['static_cum'], 4)])
